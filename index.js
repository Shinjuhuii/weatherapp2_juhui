$(document).ready(function () {
    ...
    function ForecastWeatherService(url) {
        $.getJSON(url, function (data) {
            let dailyWeather = data.list.filter((item, index) => index % 8 === 0).slice(0, 5);
            $('#humidity2 .text5').empty();
    
            dailyWeather.forEach(item => {
                let dayOfWeek = new Date(item.dt * 1000).toLocaleString('ko-KR', { weekday: 'short' });
                let maxTemp = Math.floor(item.main.temp_max);
                let minTemp = Math.floor(item.main.temp_min);
